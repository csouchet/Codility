 ...
 * 		- expected worst-case space complexity is O(L), beyond input storage (not counting the storage required
 * for input arguments).
 * 
...
public class Ladder {
	...
	public int[] solution(int[] A, int[] B) {
		final long[] fib = getFibonacci();

		int n = A.length;
		final int[] result = new int[n];
		for (int i = 0; i < n; i++) {
			// 1 << b is usually the same as Math.pow(2,b)
			final long pow = 1 << B[i];
			result[i] = (int) (fib[A[i] + 1] & (pow - 1));
		}
